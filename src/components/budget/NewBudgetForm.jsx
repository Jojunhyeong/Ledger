...
export default function NewBudgetForm({ open, onClose }) {
  ...
  return (
    <div className='fixed inset-0 z-50 grid place-items-center '>
      <div className='absolute inset-0 bg-black/40 blackdrop-blur-[1px]'/>
      <div
        ...
        <form onSubmit={handleSubmit} className="flex flex-col p-5 gap-4">
          ...
          <div className="flex gap-2">
            <button
              ...
              className={`px-3 py-1 rounded border border-gray-400 ${mode === 'existing' ? 'bg-gray-200' : ''}`}
            >
              기존 카테고리
            </button>
            <button
              type="button"
              onClick={() => setMode('new')}
              className={`px-3 py-1 rounded border border-gray-400 ${mode === 'new' ? 'bg-gray-200' : ''}`}
            >
              새 카테고리
            </button>
          </div>
